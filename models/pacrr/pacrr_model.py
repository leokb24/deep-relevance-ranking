...
# - Added PACRR-DRMM functionality.
...
class PACRR(MODEL_BASE):
    ...
    def build_doc_scorer(self, r_query_idf, permute_idxs):
        ...
        if p['combine'] < 0:
            ...
            if not p['td']:
                ...
                dout = Dense(1, name='dense_output')
                d1 = Dense(p['combine'], activation='relu', name='dense_1')
                d2 = TimeDistributed(Dense(p['combine'], activation='relu', name='dense_2'),
                                     input_shape=(p['maxqlen'], p['kmaxpool'] * p['winlen'] + 1))
                doc_score_layer = lambda x: dout(d1(d2((x))))
    ...
    def build(self):
        ...
        pos_inputs, pos_doc_inds, pos_bm25_score, pos_overlap_vec = self._create_inputs('pos', query_emb_mat,
                                                                                        embeddings)
        ...
        for neg_ind in range(p['numneg']):
            neg_inputs[neg_ind], neg_doc_inds, neg_bm25_score, neg_overlap_vec = self._create_inputs('neg%d' % neg_ind,
                                                                                                     query_emb_mat,
                                                                                                     embeddings)
            if p['context']:
