...
def produce_pos_neg_pairs(data, docset, idf_dict, term2ind, model_params, query_preprocessing, doc_preprocessing):
    ...
    for q in data['queries']:
        ...
        query_inds, query_tokens = query_preprocessing(q['query_text'], model_params['maxqlen'], idf_dict, max_idf,
                                                       term2ind)
...
def rerank(queries_to_rerank, filename, scoring_model, qrels_file, model_params, metrics, retr_dir, doc_id_prefix):
    ...
    for q in tqdm(queries_to_rerank, desc='queries'):
        scores = scoring_model.predict(
            {
                'query_inds': pad_sequences(np.tile(q['token_inds'], (len(q['retrieved_documents']['doc_list']), 1)),
                                            maxlen=model_params['maxqlen'], padding=model_params['padding_mode']),
                'query_idf': np.expand_dims(
                    pad_sequences(np.tile(q['idf'], (len(q['retrieved_documents']['doc_list']), 1)),
                                  model_params['maxqlen'], padding=model_params['padding_mode']), axis=-1),
                'pos_doc_inds': pad_sequences(q['retrieved_documents']['doc_list'], maxlen=model_params['simdim'],
                                              padding=model_params['padding_mode']),
    ...
    reported_results = {'map': trec_eval_metrics['map'], 'P_10': trec_eval_metrics['P_10'],
                        'P_20': trec_eval_metrics['P_20'], 'ndcg_cut_10': trec_eval_metrics['ndcg_cut_10'],
...
def pacrr_train(train_pairs, dev_pairs, queries_to_rerank, term2ind, config, model_params, metrics, retr_dir,
    ...
    class Evaluate(keras.callbacks.Callback):
        def on_epoch_end(self, epoch, logs={}):
            ...
            dev_rerank_results = rerank(queries_to_rerank, 'dev_ranking_epoch{0}'.format(epoch), scoring_model,
                                        config['QRELS_DEV'], model_params, metrics, retr_dir, doc_id_prefix)
            dev_res_str = results_to_string('Dev', metrics, dev_rerank_results)

            print('Epoch: {0} \n{1} \nTraining_loss: {2} \t Training_acc: {3} \nVal_loss: {4} \t Val_Acc: {5}'.format(
                epoch, '\n'.join(dev_res_str.split('\t')), logs['loss'], logs['acc'], logs['val_loss'],
                logs['val_acc']))
            log('|'.join(list(map(str, [epoch, dev_rerank_results['map'], dev_rerank_results['P_10'],
                                        dev_rerank_results['ndcg_cut_10'], dev_rerank_results['P_20'],
                                        dev_rerank_results['ndcg_cut_20'], logs['val_acc'], logs['val_loss'],
                                        logs['acc'], logs['loss']]))), retr_dir, 'log.txt')
    ...
    log('epoch|dev_map|dev_P_10|dev_ndcg_10|dev_P_20|dev_ndcg_20|dev_acc|dev_loss|train_acc|train_loss', retr_dir,
        'log.txt')
    log('epoch|test_map|test_P_10|test_ndcg_10|test_P_20|test_ndcg_20', retr_dir, 'test_log.txt')

    train_params['query_inds'] = pad_sequences(train_pairs['queries'], maxlen=model_params['maxqlen'],
                                               padding=model_params['padding_mode'])
    train_params['query_idf'] = np.expand_dims(
        pad_sequences(train_pairs['queries_idf'], maxlen=model_params['maxqlen'], padding=model_params['padding_mode']),
        axis=-1)
    train_params['pos_doc_inds'] = pad_sequences(train_pairs['pos_docs'], maxlen=model_params['simdim'],
                                                 padding=model_params['padding_mode'])
    ...
    for n in range(model_params['numneg']):
        train_params['neg{0}_doc_inds'.format(n)] = pad_sequences(train_pairs['neg_docs'],
                                                                  maxlen=model_params['simdim'],
                                                                  padding=model_params['padding_mode'])
    ...
    dev_params = {}
    dev_params['query_inds'] = pad_sequences(dev_pairs['queries'], maxlen=model_params['maxqlen'],
                                             padding=model_params['padding_mode'])
    dev_params['query_idf'] = np.expand_dims(
        pad_sequences(dev_pairs['queries_idf'], maxlen=model_params['maxqlen'], padding=model_params['padding_mode']),
        axis=-1)
    dev_params['pos_doc_inds'] = pad_sequences(dev_pairs['pos_docs'], maxlen=model_params['simdim'],
                                               padding=model_params['padding_mode'])
    ...
    for n in range(model_params['numneg']):
        dev_params['neg{0}_doc_inds'.format(n)] = pad_sequences(dev_pairs['neg_docs'], maxlen=model_params['simdim'],
                                                                padding=model_params['padding_mode'])
...
def pacrr_predict(scoring_model, weights_path, best_epoch, queries_to_rerank, term2ind, config, model_params, metrics,
    ...
    rerank_res = rerank(queries_to_rerank, 'test_ranking_epoch{0}'.format(best_epoch), scoring_model,
                        config['QRELS_TEST'], model_params, metrics, retr_dir, doc_id_prefix)
    res_str = results_to_string('Test', metrics, rerank_res)
    print('Test evaluation:')
    print('Best epoch on dev: {0} \n{1} '.format(best_epoch, '\n'.join(res_str.split('\t'))))
    log('|'.join(list(map(str, [best_epoch, rerank_res['map'], rerank_res['P_10'], rerank_res['ndcg_cut_10'],
                                rerank_res['P_20'], rerank_res['ndcg_cut_20']]))), retr_dir, 'test_log.txt')
